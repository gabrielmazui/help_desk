...
static void copiarPorUsuario(fila ** f, filaPrioridade ** fp, filaDupla ** fd, filaDupla ** fAndamento){

    noFila *atualFilaAberto = estruturasGlobais.chamadosAbertosSemPrioridade ? estruturasGlobais.chamadosAbertosSemPrioridade->first : NULL;
    noDuplo *atualFilaFechado = estruturasGlobais.chamadosSuspensos ? estruturasGlobais.chamadosSuspensos->inicio : NULL;
    noDuplo *atualFilaAndamento = estruturasGlobais.chamadosAndamento ? estruturasGlobais.chamadosAndamento->inicio : NULL;
    chamado** vetPrioridadeTemp = vetorOrdenadoFilaPrioridade(estruturasGlobais.chamadosAbertosComPrioridade);

    while(atualFilaAberto != NULL){
        chamado * c = (chamado*)atualFilaAberto->dado;
        if(strcmp(c->criador, usuario.usuario) == 0){
            filaInserir(*f, c);
        }
        atualFilaAberto = atualFilaAberto->prox;
    }

    while(atualFilaAndamento != NULL){
        chamado * c = (chamado*)atualFilaAndamento->dado;
        if(strcmp(c->criador, usuario.usuario) == 0){
            filaDuplaInserir(*fAndamento,(*fAndamento)->n, c);
        }
        atualFilaAndamento = atualFilaAndamento->prox;
    }

    while(atualFilaFechado != NULL){
        chamado * c = (chamado*)atualFilaFechado->dado;
        if(strcmp(c->criador, usuario.usuario) == 0){
            filaDuplaInserir(*fd,(*fd)->n, c);
        }
        atualFilaFechado = atualFilaFechado->prox;
    }

    int q = 0;
    while(q < estruturasGlobais.chamadosAbertosComPrioridade->n){
        chamado * c = vetPrioridadeTemp[q];
        if(strcmp(c->criador, usuario.usuario) == 0){
...
static void carregarBufferChamados(char ***buf1, char ***buf2, char ***buf3, int* linhasAberto, int* linhasAndamento, int* linhasFechado, int*** divisaoLinhas, int* totalPaginas) {
    ...
    *buf3 = NULL;
    ...
    for (int i = 0; i < fQuant + fpQuant; i++) {
        int limite = (paginasAberto == 1) ? 30 : 40;
    ...
    for (int i = 0; i < fAndamentoQuant; i++) {
        int limite = (paginasAndamento == 1) ? 30 : 40;
    ...
    for (int i = 0; i < fdQuant; i++) {
        int limite = (paginasFechado == 1) ? 30 : 40;
...
static void printarBufferChamados(int **divisaoLinhas, char **buf1, char **buf2, char **buf3, int linhas1, int linhas2, int linhas3, int paginaAtual, int totalPaginas, int selected){
    ...
    if(paginaAtual == 1){
        // primeira pagina tem 10 linhas a menos (header)
    ...
    int fimLoop = (paginaAtual == 1) ? 40 - 10 : 40; // primeira pagina tem 10 linhas a menos (header)
...
void clientesChamados(void){
    ...
    while(1){
        ...
        if(paginaAtual == 1){
           if(k == KC_DOWN){
                if(selected < 3){  // máximo 3 opções
                    // desmarcar opção atual
                    updateOption(selected + 2, opcoes[selected-1], "", (selected == 2 ? RED : BLUE));
                    selected++;
                    // marcar opção nova
                    if(selected == 3){
                        // pagina
                        ...
                        updateOption(selected + 2, opcoes[selected-1], (selected == 2 ? BG_RED : BG_BLUE), "");
                    }
                }
            }else if(k == KC_UP){
                if(selected > 1){
            ...
            }else if(k == KC_RIGHT && selected == 3){
                if(paginaAtual < totalPaginas){
                    paginaAtual++;
                    clear();
                    printarBufferChamados(divisaoLinhas, matrizAberto, matrizAndamento, matrizFechado, linhasAberto, linhasAndamento, linhasFechado, paginaAtual, totalPaginas, selected);
                }
            }else if(k == KC_LEFT && selected == 3){
                if(paginaAtual > 1){
                    paginaAtual--;
                    clear();
                    printarBufferChamados(divisaoLinhas, matrizAberto, matrizAndamento, matrizFechado, linhasAberto, linhasAndamento, linhasFechado, paginaAtual, totalPaginas, selected);
